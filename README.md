...
- **Нарисовать картинку** (спрайт, фон, иконку): `image_generate {prompt, repo: "dos-gatitos", role_id, path}` — промпт уйдёт
  в ChatGPT владельца (в его браузере), готовый файл ляжет в твою копию по `path` (например `assets/images/cat_run.png`).
  Генерация 1-2 мин, итог — `image_status`. Пиши промпт полностью: что, стиль, цвета, ракурс, «прозрачный фон» для спрайтов;
  одни и те же слова про стиль во всех промптах. Потом проверь размер (python3 + PIL), при надобности уменьши, и `repo_publish`.
